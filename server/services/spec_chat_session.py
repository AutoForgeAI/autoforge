...
class SpecChatSession:
    ...
    def __init__(self, project_name: str, project_dir: Path, from_research: bool = False):
        """
        ...
        Args:
            ...
            from_research: Whether this session was started from research results
                           (existing codebase analysis). When True, the system prompt
                           is augmented with research documents and the conversation
                           starts in "existing codebase" mode.
        """
    ...
    async def start(self) -> AsyncGenerator[dict, None]:
        ...
        # If coming from research results, prepend the research preamble to the system prompt.
        # This instructs Claude to skip "new project" questions and instead work with
        # the existing codebase context from the research analysis documents.
        if self.from_research:
            research_preamble = self._build_research_preamble()
            if research_preamble:
                system_prompt = research_preamble + "\n\n" + system_prompt
                logger.info("Prepended research preamble to system prompt for existing codebase mode")
        ...
        # Start the conversation - Claude will send the appropriate greeting
        # For existing codebases, instruct Claude to use the research context
        initial_message = (
            "Begin the spec creation process for this existing codebase. "
            "The research analysis documents are included in your system prompt "
            "under RESEARCH ANALYSIS DOCUMENTS. Summarize what you found and ask "
            "the user what they want to add, change, or improve."
            if self.from_research
            else "Begin the spec creation process."
        )
        try:
    ...
    def _build_research_preamble(self) -> str | None:
        """
        Read research analysis docs and build a preamble for existing codebase mode.

        When the user arrives from the research results page, the .planning/codebase/
        directory contains analysis documents (STACK.md, ARCHITECTURE.md, etc.)
        generated by the research agent. This method reads those documents and builds
        a preamble that is prepended to the system prompt, instructing Claude to skip
        the "new project" questions and instead ask what the user wants to change.

        Returns:
            A preamble string to prepend to the system prompt, or None if no
            research documents were found.
        """
        docs_dir = self.project_dir / ".planning" / "codebase"
        if not docs_dir.exists():
            return None

        preamble = """# EXISTING CODEBASE MODE

IMPORTANT: This is NOT a new project. The user has an EXISTING codebase that has already been analyzed by the research agent. The research analysis documents are included below.

## MODIFIED CONVERSATION FLOW

Since this is an existing codebase:
- You ALREADY KNOW the project name, tech stack, architecture, structure, conventions, and integrations from the research docs below.
- Do NOT ask Phase 1 questions (project name, description, audience) - you already have this information.
- Do NOT ask Phase 3 questions (tech preferences) - the stack is already established.
- Skip straight to asking what the user wants to ADD, CHANGE, or IMPROVE.
- Phase 4 (Features) should focus on NEW features or CHANGES to existing functionality.
- When generating the spec, include the existing architecture and stack as context.

## GREETING FOR EXISTING CODEBASE

Instead of the standard greeting, start by briefly summarizing what you found in the codebase analysis (project name, key technologies, main architecture patterns), then ask:

> "I've analyzed your existing codebase. Here's a quick summary of what I found:
>
> **Stack:** [summarize from STACK.md - key frameworks and languages]
> **Architecture:** [summarize from ARCHITECTURE.md - main patterns]
>
> What would you like to do with this project?
>
> 1. **Add new features** - Extend with new functionality
> 2. **Refactor or improve** - Clean up, modernize, or optimize existing code
> 3. **Fix issues** - Address bugs, performance, or security concerns
> 4. **A combination** - Multiple types of changes"
>
> STOP HERE and wait for their response. Do not ask any other questions yet.
...
async def create_session(
    project_name: str, project_dir: Path, from_research: bool = False
) -> SpecChatSession:
    ...
    with _sessions_lock:
        ...
        session = SpecChatSession(project_name, project_dir, from_research=from_research)
